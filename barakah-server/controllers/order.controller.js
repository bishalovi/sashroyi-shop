...
exports.createOrder = async (req, res) => {
  try {
    ...
    // Non-blocking Server-Side Tracking (CAPI) Dispatch
    setImmediate(async () => {
      try {
        const trackingCollection = db.collection("tracking_settings");
        const trackingConfig = await trackingCollection.findOne({ key: "main_tracking_config" });
        if (trackingConfig) {
          const clientIp = req.headers["x-forwarded-for"] || req.socket.remoteAddress;
          const userAgent = req.headers["user-agent"];
          const fullOrder = { _id: result.insertedId, ...orderData, ...req.body };

          if (trackingConfig.facebook?.isEnabled && trackingConfig.facebook?.isCapiEnabled) {
            sendMetaCapiPurchase({
              order: fullOrder,
              metaConfig: trackingConfig.facebook,
              clientIp,
              userAgent,
            }).catch((e) => console.error("Meta CAPI dispatch error:", e.message));
          }

          if (trackingConfig.tiktok?.isEnabled && trackingConfig.tiktok?.isCapiEnabled) {
            sendTikTokEventsApiPurchase({
              order: fullOrder,
              tiktokConfig: trackingConfig.tiktok,
              clientIp,
              userAgent,
            }).catch((e) => console.error("TikTok CAPI dispatch error:", e.message));
          }
        }
      } catch (err) {
        console.error("CAPI Background trigger error:", err.message);
      }
    });
  } catch (error) {
