...
export default function CheckoutPage() {
  ...
  const [shippingConfig, setShippingConfig] = useState({
    insideDhaka: { title: "ঢাকার ভিতরে", cost: 60, isEnabled: true },
    outsideDhaka: { title: "ঢাকার বাইরে", cost: 120, isEnabled: true },
    freeShipping: { title: "ফ্রি ডেলিভারি", cost: 0, isEnabled: false },
  });
  ...
  useEffect(() => {
    if (!baseUrl) return;
    fetch(`${baseUrl}/api/shipping/public`)
      .then((res) => res.json())
      .then((res) => {
  ...
  const shippingCost = useMemo(() => {
    if (shipping === "inside") {
      return shippingConfig.insideDhaka?.isEnabled ? Number(shippingConfig.insideDhaka.cost || 0) : 0;
    }
    if (shipping === "outside") {
      return shippingConfig.outsideDhaka?.isEnabled ? Number(shippingConfig.outsideDhaka.cost || 0) : 0;
    }
    if (shipping === "free") {
      return 0;
    }
  ...
  return (
    <main className="min-h-screen bg-[#f8f6f1] py-12">
      ...
      <Container>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.5fr_0.9fr]">
            ...
            >
              ...
              <div className="mt-8">
                ...
                <div className="space-y-3">
                  {shippingConfig.insideDhaka?.isEnabled && (
                    <label className={`flex cursor-pointer items-center justify-between rounded-xl border px-4 py-4 transition ${
                      shipping === "inside"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-[#0f2a44]">
                          {shippingConfig.insideDhaka.title || "ঢাকার ভিতরে"}
                        </span>
                      </div>
                      <span className="font-semibold text-[#0f2a44]">
                        ৳ {Number(shippingConfig.insideDhaka.cost || 0).toFixed(2)}
                      </span>
                    </label>
                  )}

                  {shippingConfig.outsideDhaka?.isEnabled && (
                    <label className={`flex cursor-pointer items-center justify-between rounded-xl border px-4 py-4 transition ${
                      shipping === "outside"
                        ? "border-[#0f2a44] bg-[#0f2a44]/5 shadow-sm"
                        : "border-[#0f2a44]/10 hover:border-[#0f2a44]/30"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className="font-medium text-[#0f2a44]">
                          {shippingConfig.outsideDhaka.title || "ঢাকার বাইরে"}
                        </span>
                      </div>
                      <span className="font-semibold text-[#0f2a44]">
                        ৳ {Number(shippingConfig.outsideDhaka.cost || 0).toFixed(2)}
                      </span>
                    </label>
                  )}

                  {(shippingConfig.freeShipping?.isEnabled || hasFreeShippingItem) && (
                    <label className={`flex cursor-pointer items-center justify-between rounded-xl border px-4 py-4 transition ${
                      shipping === "free"
                        ? "border-emerald-600 bg-emerald-50/50 shadow-sm"
                        : "border-[#0f2a44]/10 hover:border-emerald-500/30"
                    }`}>
                      <div className="flex items-center gap-3">
                        <input
                          type="radio"
                        ...
                        <span className="font-medium text-emerald-800">
                          {shippingConfig.freeShipping.title || "ফ্রি ডেলিভারি"}
                        </span>
                      </div>
        ...
        <div className="fixed bottom-5 right-5 z-9999 flex flex-col gap-3 items-end">
          {open && (
            <>
              {/* Messenger */}
              <a
                href={contact?.messengerUrl || "https://m.me/"}
                target="_blank"
              ...
              {/* WhatsApp */}
              <a
                href={getWhatsAppUrl("হ্যালো! আমি চেকআউট এবং অর্ডার সংক্রান্ত বিষয়ে জানতে চাচ্ছি।")}
                target="_blank"
                rel="noopener noreferrer"
                className="btn btn-lg btn-circle bg-white shadow-lg hover:scale-105 transition"
              >
              ...
              {/* Phone */}
              <a
                href={`tel:${cleanPhoneNumber || "01910037935"}`}
                className="btn btn-lg btn-circle bg-white shadow-lg hover:scale-105 transition"
              >
        ...
        <a
          href={`tel:${cleanPhoneNumber || "01910037935"}`}
          className="w-40 md:w-48 mt-4 flex items-center justify-center gap-2 mx-auto rounded-xl bg-[#0f2a44] px-6 py-3 text-xs md:text-lg font-semibold text-[#f2c94c] transition hover:opacity-95"
        >
          <FaPhoneAlt />
          এখনই কল করুন
        </a>
